...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (devices.isNotEmpty && rooms.isNotEmpty) {
      ...
      final List<Device> tempSensors = devices
          .where((e) =>
              DeviceType.getDeviceType(e.deviceId) == DeviceType.tempSensor)
          .toList();
    ...
    return SingleChildScrollView(
      ...
      child: Padding(
        ...
        child: devices.isNotEmpty && rooms.isNotEmpty
            ? SizedBox(
                width: double.infinity,
                height: devices.isEmpty
                    ? 400
                    : 91 + 160 + (devices.length / 2).ceil() * 158,
                child: Column(
                  children: [
                    const Song(),
                    Expanded(
                    ...
                    if (tempSensor != null) TempSensorWidget(device: tempSensor)
                  ],
                ),
              )
            : const SizedBox(),
